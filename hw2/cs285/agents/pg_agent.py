...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rews_list):
        ...
        if not self.reward_to_go:
            ...
            q_values = np.concatenate([self._discounted_return(rollout) for rollout in rews_list]) #TODO
        ...
        # Case 2: reward-to-go PG 
            ...
            # Hint3: see the helper functions at the bottom of this file
            q_values = np.concatenate([self._discounted_cumsum(rollout) for rollout in rews_list])
    ...
    def estimate_advantage(self, obs, q_values):
        ...
        if self.nn_baseline:
            b_n_unnormalized = self.actor.run_baseline_prediction(obs=obs)
    ...
    def _discounted_return(self, rewards):
        ...
        indices = np.arange(len(rewards)) #TODO

        # 2) create a list where the entry at each index (t') is gamma^(t')
        discounts = [np.power(self.gamma,indices)] #TODO

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = rewards*discounts #TODO

        # 4) calculate a scalar: sum_{t'=0}^{T-1} gamma^(t') * r_{t'}
        sum_of_discounted_rewards = np.sum(discounted_rewards)#TODO

        # 5) create a list of length T-1, where each entry t contains that scalar
        list_of_discounted_returns = np.ones(len(rewards))*sum_of_discounted_rewards #TODO
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            indices = np.arange(len(rewards)-start_time_index) #TODO
            ...
            discounted_rtg = rewards[start_time_index:]*discounts #TODO

            # 4) calculate a scalar: sum_{t'=t}^{T-1} gamma^(t'-t) * r_{t'}
            sum_discounted_rtg = np.sum(discounted_rtg) #TODO
