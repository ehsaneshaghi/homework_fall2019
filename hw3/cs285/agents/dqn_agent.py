...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        if perform_random_action:
            ...
            # previous frames.
            enc_last_obs = self.replay_buffer.encode_recent_observation()#TODO
            enc_last_obs = enc_last_obs[None, :]

            # TODO query the policy with enc_last_obs to select action
            action = self.actor.get_action(enc_last_obs)#TODO
            action = action[0]
        ...
        obs,reward,done,info = self.env.step(action) #TODO
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts and \
            ...
            obs_t_ph, act_t_ph, rew_t_ph, obs_tp1_ph, done_mask_ph = self.sample(self.batch_size) #TODO
            feed_dict = {
                self.critic.learning_rate: self.optimizer_spec.lr_schedule.value(self.t),
                self.critic.obs_t_ph:obs_t_ph, #TODO
                self.critic.act_h_ph:act_t_ph, #TODO
                self.critic.rew_t_ph:rew_t_ph, #TODO
                self.critic.obs_tp1_ph = obs_tp1_ph, #TODO
                self.critic.done_mask_ph = done_mask_ph, #TODO
            }
